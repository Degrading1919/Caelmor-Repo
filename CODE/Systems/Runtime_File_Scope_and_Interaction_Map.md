...
- Persistence: persistence write requests are enqueued (`PersistenceWriteQueue`) with per-player/global caps. I/O happens off-thread (adapter not present); apply-on-tick is expected to drain completion queues during mutation windows.
...
- Transport thread(s): assumed to live in `PooledTransportRouter`/network layer (not included); only interact via bounded queues (`DeterministicTransportRouter`, `BoundedReplicationSnapshotQueue`, `AuthoritativeCommandIngress`). Off-thread code must not mutate world state.
...
-----------------------
- Networking Transport & Message Routing: not implemented. Routing placeholders exist in `RuntimeServerLoop` (`PooledTransportRouter`) and queue definitions in `DeterministicTransportQueues`. Entry points: `RouteInbound`, `RouteSnapshot`, `TryDequeueSnapshot`. Dependencies: `RuntimeBackpressureConfig`, session IDs, snapshot serializer. Invariants: bounded per-session queues, pooled buffers; tick thread must be sole state mutator. Status: transport thread/workers UNKNOWN.
- Connection/Session Handshake Pipeline: represented by `SessionHandshakePipeline` referenced in `RuntimeServerLoop`; concrete handshake processing absent. Entry points: Start/Stop lifecycle and disconnect handling in `RuntimeServerLoop`. Dependencies: onboarding/session systems. Invariants: handshake mutation must occur on tick thread; mailboxes expected to be bounded. Status: implementation UNKNOWN.
...
- Persistence IO Hooks: persistence request DTO (`PersistenceWriteRequest`) and bounded queue (`PersistenceWriteQueue`) exist; actual I/O worker/apply-on-tick adapter missing. Entry points: `Enqueue`, `TryDequeue`. Dependencies: `RuntimeBackpressureConfig`, player IDs. Invariants: per-player/global caps, drop-oldest policy, no blocking. Status: I/O worker UNKNOWN.

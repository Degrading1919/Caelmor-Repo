# Skill Level & Mastery Philosophy

**Creative Director status:** approved for skill progression design. This document defines the numerical cap, milestone identities, closed time-to-milestone targets, the non-combat fixed-base-XP architecture, the selected generalized-exponential curve family, the final coefficients A = 8 and K = 15, the 5,000,000 XP level-99 display scale, and the resulting authoritative 1–99 threshold table. It does not set final activity XP values, XP/hour, action timing, RNG, individual unlock positions, or runtime behavior.

## Skill Cap

- Skill cap: **99**.
- Ordinary levels are satisfying progress pulses; they do not each require an authored unlock.
- Governing rule: **A level does not require an unlock. A milestone requires meaning.**
- The 99-level horizon is intended to support long-term attachment, personal goals, and remembered progression while avoiding filler content.

## Progression Identities

| Levels | Identity | Meaning |
|---|---|---|
| 1–30 | **Novice** | Learn the discipline, basic loop, places, materials, tools, and routines. |
| 31–50 | **Competent** | Useful independence and meaningful choices. |
| 51–70 | **Advanced** | Knowledge, routing, preparation, cross-skill understanding, and efficiency distinguish the player. |
| 71–98 | **Masterful** | Substantial command; refinement and specialization. |
| ~80 | **Practical mastery point** | The player should feel almost mastered; most core practical functionality is available. |
| 99 | **Cap / Completion** | Primarily identity and commemoration. |

## Upper-Level Philosophy

- Around level 80, the player should feel practically mastered.
- Levels 80–98 emphasize refinement, specialization, exceptional opportunities, personal goals, and dedication.
- A small number of post-80 functions may be extremely rewarding, but they are specialist capabilities rather than missing essentials.
- Level 99 should feel memorable and personally significant without requiring a mandatory final power spike.

## XP Progression Philosophy

Caelmor uses a **RuneScape-like generalized exponential progression family**, independently tuned for Caelmor rather than copying the OSRS or RS3 XP table. The selected family is intended to preserve increasingly meaningful levels and a prestigious late journey while still supporting practical mastery around level 80.

The progression target is a **long-form level-99 journey of approximately 135–150 engaged skill hours**, using **145 hours as the nominal analytical target**.

"Engaged skill hours" means ideal time actively progressing that discipline. It does not represent elapsed account time and excludes travel, downtime, market activity, inventory friction, quests, combat interruptions, social play, and other non-training activity.

### Nominal milestone targets

| Milestone | Identity | Nominal engaged time from level 1 | Early tuning envelope |
|---|---|---:|---:|
| 30 | Novice complete | **~11 h** | ~10–12 h |
| 50 | Competent complete | **~31 h** | ~28–34 h |
| 70 | Advanced complete | **~63 h** | ~58–68 h |
| 80 | Practical mastery | **~90 h** | ~85–92 h |
| 99 | Cap / Completion | **~145 h** | ~135–150 h |

Practical mastery therefore occurs at roughly **62% of the nominal journey**, leaving roughly **38%** of the commitment after level 80 for refinement, specialization, prestige, identity, and long-term personal goals.

### Closed XP display scale

The working 1–99 progression uses:

```text
XP at level 99 = 5,000,000
```

This is an **absolute XP scale**, not a pacing formula. Changing the numeric scale alone does not change the normalized curve shape or the closed time targets.

The 5,000,000 scale is intended to provide enough integer granularity for stable authored activity rewards without pushing ordinary non-combat actions into unnecessarily inflated numbers.

### Closed curve formula, coefficients, and threshold table

The authoritative progression formula is:

```text
Weight(L) = L + 8 × 2^(L / 15)

RawXP(L) = Σ Weight(i), for i = 1 to L-1

RequiredXP(L)
    = round(
        5,000,000 × RawXP(L) / RawXP(99)
      )
```

Closed parameters:

```text
A = 8
K = 15
XP99 = 5,000,000
```

The exact 1–99 cumulative thresholds are authoritative in:

```text
03_CORE_SYSTEMS/Skills & XP v1/Caelmor_XP_Threshold_Table.csv
```

Key thresholds are:

| Level | Cumulative XP |
|---:|---:|
| 30 | 221,617 |
| 50 | 652,858 |
| 70 | 1,549,627 |
| 80 | 2,324,814 |
| 90 | 3,475,278 |
| 99 | 5,000,000 |

Level 80 sits at **46.49628%** of total level-99 XP. The remaining raw XP after practical mastery is intentional and is reconciled with the closed engaged-time targets through more productive later methods rather than hidden player-level XP scaling.

The generated table has monotonic XP-to-next-level growth from **2,224 XP for 1→2** to **198,978 XP for 98→99**, with no flat/reversed level costs and no artificial final-level multiplier.

### Fixed non-combat activity XP architecture

For authored non-combat skilling/economy activities:

- the same authored action under the same conditions has the same base XP regardless of player level;
- player level itself does not invisibly multiply base XP;
- different activities may legitimately award different fixed base XP because they are genuinely different activities;
- higher-level methods can improve XP/hour through better resources, tools, routes, access, preparation, knowledge, or explicitly authorized modifiers;
- target XP/hour is an analytical result or diagnostic, not the automatic source of per-action XP.

For example, mining the same iron deposit under the same conditions should retain the same base XP at level 20 and level 90. A level-90 player may progress faster because of better tools, routes, access, or superior alternative activities—not because iron silently becomes worth more XP.

This rule applies to non-combat skilling/economy XP. It does not define combat XP, quest XP, encounter XP, or other future reward models.

### Intended pacing feel

- **1–10:** rapid orientation; early progress should feel dependable and welcoming.
- **11–30:** steady apprenticeship; progress remains energetic while levels gain personal weight.
- **31–50:** established competence; the skill becomes a dependable part of the character.
- **51–70:** serious investment; knowledge and efficiency increasingly matter.
- **71–80:** mastery approach; the player should feel visibly close to practical mastery.
- **81–90:** refinement; progression becomes more deliberate and identity-driven.
- **91–98:** dedication; each level is meaningful without becoming an artificial punishment wall.
- **99:** culmination; the achievement supplies the emotional spike, so 98→99 should not be grotesquely disproportionate to surrounding late levels.

### Method-discovery principle

The nominal time targets are not fixed-rate promises. A player who learns better routes, preparation, resource relationships, tool choices, processing chains, or other legitimate skill-specific efficiencies should be able to progress faster than a less-informed player.

Future XP/hour tuning should therefore support a **baseline method plus a meaningful but controlled efficiency ceiling**, preserving knowledge-based mastery without allowing optimal methods to trivialize the journey.

### Practical-mastery boundary

By approximately level 80:

- most core practical functionality should be available;
- the skill should feel complete enough for confident ordinary high-level use;
- remaining unlocks should be specialist, prestigious, unusually efficient, or otherwise exceptional;
- no fundamental feature necessary to understand or enjoy the discipline should be withheld simply to populate levels 81–98.

## Expansion Rule

Future skill-cap increases remain legitimate. If a skill rises beyond 99 later, 99 remains a historical mastery landmark and should not be retroactively treated as false mastery.

## Content-Density Guardrail

The level count must never force filler resources, recipes, tools, tiers, or catalog bloat. A skill's identity should come from meaningful mechanics, world memories, useful cross-skill connections, knowledge-based efficiency, and selected authored landmarks.

## Balance Boundary

The closed philosophy above does **not** yet define:

- baseline or optimal XP/hour;
- action durations;
- final XP per gather/craft/completion;
- success or failure probabilities;
- resource yield rates;
- exact unlock levels inside each identity band;
- runtime formulas or implementation.

Those values should be derived later from the milestone targets and validated against actual world friction, input supply, economy throughput, skill-specific activity structure, and playtest results.

---

## GATHERING / WORLD INTERACTION SKILLS

---

## Foraging

### Economic Role
Foraging supplies all living-terrain inputs that cannot be extracted through force or industry.
It is the backbone of subtle power systems and long-term stability.

### What Breaks If It Is Untrained
- Alchemy stagnates at low tiers
- Cooking loses regional depth and endurance paths
- Exploration becomes economically unrewarding

### Resource Roles Supplied
- Nutritional
- Stabilizing
- Catalytic
- Binding

### Downstream Dependencies
- Cooking (nutritional, binding)
- Alchemy (catalytic, stabilizing)
- Mechanisms (binding compounds)

### Regional Identity
- **Thrives:** Lowmark, Mire
- **Struggles:** Emberholt
- **Culturally Supported:** Lowmark (quiet sustenance)
- **Culturally Suppressed:** Emberholt (non-industrial value)

### World Rationale
Lowmark’s restraint during the war preserved its land.
The Mire’s grief altered growth in dangerous but valuable ways.
Foraging reflects listening to the land rather than extracting from it.

---

## Hunting

### Economic Role
Hunting is the sole source of intact biological structure.
It controls the supply of flexible, resilient, and beast-derived materials.

### What Breaks If It Is Untrained
- Leatherworking collapses
- Fletching loses advanced paths
- Adornment loses beast-based modifiers

### Resource Roles Supplied
- Structural
- Binding
- Catalytic
- Conductive (biological / kinetic)

### Downstream Dependencies
- Leatherworking (structural, binding)
- Fletching (structural, binding)
- Adornment (catalytic modifiers)
- Alchemy (catalytic inputs)

### Regional Identity
- **Thrives:** Thornfell, Mire
- **Struggles:** Emberholt
- **Culturally Supported:** Thornfell (self-sufficiency)
- **Culturally Neutral:** Lowmark

### World Rationale
War reshaped fauna in Thornfell and the Mire.
Hunting there is dangerous, respected, and economically essential.

---

## Angling

### Economic Role
Angling governs endurance economies and fluid-based systems.
It is the only path to sustained, long-duration support.

### What Breaks If It Is Untrained
- Cooking loses endurance chains
- Alchemy loses fluid reaction paths
- Long expeditions become unsustainable

### Resource Roles Supplied
- Nutritional
- Stabilizing
- Catalytic
- Conductive (fluid-borne)

### Downstream Dependencies
- Cooking (nutritional, stabilizing)
- Alchemy (catalytic, conductive)

### Regional Identity
- **Thrives:** Lowmark
- **Struggles:** Thornfell, Emberholt
- **Culturally Supported:** Lowmark
- **Culturally Ignored:** Emberholt

### World Rationale
Lowmark’s rivers carried the war’s memory.
Angling is treated as patient endurance, not extraction.

---

## Mining

### Economic Role
Mining is the backbone of hard progression.
It defines all metallic ceilings in the game.

### What Breaks If It Is Untrained
- Smithing halts
- Mechanisms stagnate
- Fletching loses penetrative paths

### Resource Roles Supplied
- Structural
- Conductive
- Catalytic
- Stabilizing

### Downstream Dependencies
- Smithing
- Mechanisms
- Fletching
- Adornment (stabilizing matrices)

### Regional Identity
- **Thrives:** Thornfell, Emberholt
- **Struggles:** Lowmark
- **Culturally Supported:** Thornfell, Emberholt

### World Rationale
Thornfell’s ridges and Emberholt’s volcanic depths absorbed war through stone.
Mining is respected, dangerous, and politically charged.

---

## Woodcutting

### Economic Role
Woodcutting supplies flexible structure and kinetic conduction.
It enables scale without rigidity.

### What Breaks If It Is Untrained
- Fletching collapses
- Mechanisms lose shock absorption
- Fabrication bottlenecks

### Resource Roles Supplied
- Structural
- Binding
- Conductive (elastic / kinetic)
- Stabilizing

### Downstream Dependencies
- Fletching
- Mechanisms
- Fabrication

### Regional Identity
- **Thrives:** Lowmark, Thornfell
- **Struggles:** Mire
- **Culturally Supported:** Lowmark

### World Rationale
Forests survived where restraint existed.
Woodcutting reflects selective harvest, not clear-cutting.

---

## Scavenging

### Economic Role
Scavenging injects legacy, anomaly, and unpredictability into the economy.
It is the only backward-facing progression input.

### What Breaks If It Is Untrained
- Mechanisms lose purpose
- Adornment loses rare paths
- Ruins and warzones become dead content

### Resource Roles Supplied
- Structural (irregular)
- Binding
- Conductive
- Catalytic

### Downstream Dependencies
- Mechanisms
- Adornment
- Fabrication

### Regional Identity
- **Thrives:** Mire, Emberholt ruins
- **Dangerous:** Everywhere
- **Culturally Feared:** Lowmark
- **Secretly Exploited:** Emberholt

### World Rationale
The war left behind broken systems and forbidden remnants.
Scavenging is risky because history here is unstable.

---

## CRAFTING / CREATION SKILLS

---

## Smithing

### Economic Role
Smithing defines all weapon, tool, and metal-framework ceilings.

### What Breaks If It Is Untrained
- Combat progression halts
- Tool quality caps early
- Mechanisms lack cores

### Resource Roles Required
- Structural
- Conductive
- Stabilizing

### Regional Identity
- **Thrives:** Thornfell, Emberholt
- **Culturally Central:** Thornfell
- **Politically Controlled:** Emberholt

---

## Leatherworking

### Economic Role
Leatherworking enables mobility, flexibility, and light defense economies.

### What Breaks If It Is Untrained
- Light armor paths vanish
- Fletching stabilizers disappear
- Adornment loses organic slots

### Resource Roles Required
- Structural
- Binding

### Regional Identity
- **Thrives:** Thornfell, Lowmark
- **Struggles:** Emberholt

---

## Fletching

### Economic Role
Fletching controls all consumable ranged combat loops.

### What Breaks If It Is Untrained
- Ranged combat collapses
- Resource sinks vanish
- Hunting feedback loops fail

### Resource Roles Required
- Structural
- Binding
- Conductive

### Regional Identity
- **Thrives:** Thornfell
- **Struggles:** Mire, Emberholt

---

## Alchemy

### Economic Role
Alchemy enables adaptive response and temporary system modification.

### What Breaks If It Is Untrained
- No counterplay to difficulty spikes
- Exploration risk becomes binary
- High-tier tuning collapses

### Resource Roles Required
- Catalytic
- Stabilizing
- Conductive

### Regional Identity
- **Thrives:** Lowmark, Mire
- **Politically Restricted:** Emberholt

---

## Cooking

### Economic Role
Cooking governs sustained performance and session endurance.

### What Breaks If It Is Untrained
- Long-form play becomes inefficient
- Angling economy collapses
- Non-combat mains suffer disproportionately

### Resource Roles Required
- Nutritional
- Binding
- Stabilizing

### Regional Identity
- **Thrives:** Lowmark
- **Undervalued:** Emberholt

---

## Adornment

### Economic Role
Adornment defines specialization identity and long-term mastery ceilings.

### What Breaks If It Is Untrained
- Skill ceilings flatten
- Non-combat prestige disappears
- Economic differentiation collapses

### Resource Roles Required
- Catalytic
- Stabilizing
- Conductive

### Regional Identity
- **Thrives:** Mire, Emberholt
- **Subtle Forms:** Lowmark

---

## Mechanisms

### Economic Role
Mechanisms enable access, safety, efficiency, and expedition infrastructure.

### What Breaks If It Is Untrained
- Scavenging becomes pointless
- High-risk gathering collapses
- World traversal stagnates

### Resource Roles Required
- Structural
- Conductive
- Binding

### Regional Identity
- **Thrives:** Emberholt
- **Struggles:** Mire

---

## Fabrication

### Economic Role
Fabrication is the connective tissue of the economy.
Fabrication does not create raw power; it assembles, finalizes, and adapts power created elsewhere.

### What Breaks If It Is Untrained
- Cross-skill items cannot exist
- Mechanisms and Adornment bottleneck
- Economy fragments into silos

### Resource Roles Required
- Binding
- Structural
- Conductive

### Regional Identity
- **Thrives:** Emberholt
- **Emergent Use:** Trade hubs

---
